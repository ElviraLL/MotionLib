...
class MeshRendererWrapper:
    ...
    def __init__(
        ...
        self.lights = (
            ...
            else AmbientLights(ambient_color=((0.5, 0.5, 0.5),), device=device)
        )
...
class Pyt3DWrapper:
    def __init__(
        ...
        if use_fixed_cameras:
            focal_length = torch.tensor(
                (-intrin[0, 0], -intrin[1, 1]), dtype=torch.float32
            ).unsqueeze(0)
            cam_center = torch.tensor(
                (intrin[0, 2], intrin[1, 2]), dtype=torch.float32
            ).unsqueeze(0)
            print(cam_center)
            ...
            self.cameras = self.get_surround_cameras(
                self,
                intrin,
                image_size,
                eyes=eyes,
                n_poses=len(eyes) if not eyes is None else 1,
                device=device,
            )
    ...
    def prepare_render(
        self, meshes, colors, specified_vertices, static_contact_color=[0.9, 0.2, 0.2]
    ):
        py3d_meshes = []
