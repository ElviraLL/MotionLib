...
CAMERA_TO_WORLD = np.float32(
    [
        [
            -3.362828398520266715e-01,
            4.835672948475828736e-01,
            -8.081315010410416999e-01,
            2.000948008927065480e00,
        ],
        [
            -6.864405426693195866e-02,
            -8.684100987259859572e-01,
            -4.910721884250031088e-01,
            1.749267542201616843e00,
        ],
        [
            -9.392560063341914844e-01,
            -1.096657274836312368e-01,
            3.252254337849846966e-01,
            -9.957471075610673239e-01,
        ],
        [0.0, 0.0, 0.0, 1.0],
    ]
)
...
def visualize(
    ...
    pyt3d_wrapper = Pyt3DWrapper(
        image_size=IMAGE_SIZE,
        use_fixed_cameras=True,
        intrin=INTRINSIC,
        extrin=EXTRINSIC,
        device=device,
        lights=PointLights(device=device, location=[[0.0, 4.0, 0.0]]),
    )
    ...
    for frame_idx in tqdm(range(0, N, sampling_rate)):
        ...
        obj_posed_mesh = trimesh.Trimesh(
            vertices=(obj_pts.copy() @ obj_pose[:3, :3].T) + obj_pose[:3, 3],
            faces=obj_faces.copy(),
        )
        person1_mesh = trimesh.Trimesh(
            vertices=person1_smplx_data["vertices"][frame_idx], faces=human_mesh_faces
        )
        person2_mesh = trimesh.Trimesh(
            vertices=person2_smplx_data["vertices"][frame_idx], faces=human_mesh_faces
        )
    ...
    # save
    imageio.mimsave(
        save_path, rgb_imgs, duration=(1000 / (15 // sampling_rate)), loop=0
    )
...
def process(args):
    ...
    obj_mesh = trimesh.load(
        join(object_model_root, category, obj_name + "_m.obj")
    )  # unit: m
    ...
    person1_smplx_data = np.load(
        join(motion_dir, "person1_poses.npz"), allow_pickle=True
    )["arr_0"].item()
    person2_smplx_data = np.load(
        join(motion_dir, "person2_poses.npz"), allow_pickle=True
    )["arr_0"].item()
    ...
    visualize(
        ...
        person1_smplx_data,
        person2_smplx_data,
        human_mesh_faces=human_mesh_faces,
        save_path=save_path,
        sampling_rate=1,
        device=device,
    )
...
def get_args():
    ...
    parser.add_argument(
        "--dataset_root",
        type=str,
        default="/share/datasets/hhodataset/CORE4D_release/CORE4D_Real",
    )
    parser.add_argument(
        "--object_model_root",
        type=str,
        default="/share/datasets/hhodataset/CORE4D_release/CORE4D_Real/object_models",
    )
    parser.add_argument(
        "--smplx_model_dir", type=str, default="/share/human_model/models"
    )
